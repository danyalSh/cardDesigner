		...
		if (!Modernizr.touch) {
			...
			if (settings.showCrosshair || settings.showMousePos) {
				$(window).mousemove(function(e) {
					if (settings.showCrosshair) {
						var divTop = $('.card-div').offset().top - $(document).scrollTop();
						var divLeft = $('.card-div').offset().left;
                        var divRight = $(window).width() - ($(window).width() - ($('.card-div').offset().left + $('.card-div').outerWidth()));
                        var divBottom = $('.card-div').offset().top + $('.card-div').outerHeight();
						var top = e.pageY-($(document).scrollTop())+1;
						var left = e.pageX+1;
                        if(top > divTop && left > divLeft && e.pageY < divBottom && e.pageX < divRight){
                            $('.vMouse').css("top",top);
                            $('.vMouse').css("left",divLeft+'px');
                            $('.hMouse').css("left",left);
                            $('.hMouse').css("top",divTop+'px');
                            $('.vMouse').width($('.card-div').outerWidth());
                            $('.hMouse').height($('.card-div').outerHeight());
                        }
